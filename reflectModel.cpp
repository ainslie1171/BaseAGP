...
ReflectModel::ReflectModel(ID3D11Device* device, ID3D11DeviceContext* context)
{
	m_pD3DDevice = device;
...
int ReflectModel::LoadObjModel(char* fileName)
{
	HRESULT hr = S_OK;
	...
	ID3DBlob *VS,
		*PS,
		*error;
	hr = D3DX11CompileFromFile("reflectShader.hlsl", 0, 0, "ModelVS", "vs_4_0", 0, 0, 0, &VS, &error, 0);
	...
	hr = D3DX11CompileFromFile("reflectShader.hlsl", 0, 0, "ModelPS", "ps_4_0", 0, 0, 0, &PS, &error, 0);
...
int ReflectModel::AddTexture(char* fileName)
{
	HRESULT hr = S_OK;
...
void ReflectModel::Draw(XMMATRIX& world, const XMMATRIX& view, const XMMATRIX& projection, ID3D11ShaderResourceView* skyboxTexture)//, XMVECTOR* cameraPos, LightManager* lightMgr)
{
...
void ReflectModel::CalculateModelCenterPoint()
{
...
void ReflectModel::CalculateBoundingSphereRadius()
{
...
float ReflectModel::GetBoundingSphereRadiusSqr()
{
	return m_boundingSphereRadius;// *= (m_scale*m_scale); //r2=9, 18
}

bool ReflectModel::checkCollision(ReflectModel* model)
{
